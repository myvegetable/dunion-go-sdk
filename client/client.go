...
type UnionClient interface {
	...
	GenerateCouponPwd(ctx context.Context, activityID, promotionID int64, sourceID string, opt ...model.Option) (*model.PwdResponse, error)
...
func (s client) GenerateCouponPwd(ctx context.Context, activityID, promotionID int64, sourceID string, opt ...model.Option) (*model.PwdResponse, error) {
	...
	response, err := util.Get(ctx, s.AppKey, s.AccessKey, consts.GeneratePwdUrl, body, opt...)
	if err != nil {
		return nil, err
	}
	result := &model.PwdResponse{}
	err = json.Unmarshal(response, result)
	return result, err
}
